<template>
	<view class="genghuantuandui">
		<form @submit="formSubmit(e)">
			<view class="genghuantuandui_list" v-if="type">
				<view>成员名称：</view>
				<text>{{name}}</text>
			</view>
			...
			<view class="genghuantuandui_list">
				<view>团队主管：</view>
				<text>{{team_director}}</text>
			</view>
			<view class="genghuantuandui_list">
				<view>更换团队：</view>
				<picker @change="bindPickerChange" :range="team_list" :range-key="'name'" value="id">
					<view class="uni-input">{{team_list[index].name}}-{{team_list[index].director}} </view>
				</picker>
			</view>
	...
	export default {
		...
		methods: {
			bindPickerChange: function(e) {
				...
				this.team_id = this.team_list[e.detail.value].id
			},
			...
			replace_team_info(e) {
				...
				uni.request({
					url: web_site + '/api/team/replace_team_info',
					method: 'POST',
					header: {
					...
					success: res => {
						...
						if (result.code == '0') {
							...
							_this.team_name = data.team_name;
							_this.team_director = data.team_director;
			...
			formSubmit(e) {
				...
				uni.request({
					url: web_site + '/api/team/replace_team',
					method: 'POST',
					header: {
						'content-type': 'application/x-www-form-urlencoded'
					},
